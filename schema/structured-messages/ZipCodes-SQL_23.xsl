...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:for-each select="ZIP-codes/ZIP-code">
...
INSERT INTO zip_codes (Code, NisCode
    , TownNL, TownFR, MainTownNL, MainTownFR
    , BeginValidity, EndValidity)
VALUES("<xsl:value-of select="code"/>"
        , "<xsl:value-of select="NIS-code"/>"
        , "<xsl:value-of select="town/nl"/>"
        , "<xsl:value-of select="town/fr"/>"
        , "<xsl:value-of select="main-town/nl"/>"
        , "<xsl:value-of select="main-town/fr"/>"
