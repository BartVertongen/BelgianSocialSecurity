...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:for-each select="employee-statuses/employee-status">
...
INSERT INTO employee_statuses(Code, RSZ, RSZ-PPO, DescriptionNL, DescriptionFR
    , RemarkNL, RemarkFR, BeginValidity, EndValidity)
VALUES("<xsl:value-of select="code"/>"
        , "<xsl:value-of select="rsz"/>"
        , "<xsl:value-of select="rsz-ppo"/>"
