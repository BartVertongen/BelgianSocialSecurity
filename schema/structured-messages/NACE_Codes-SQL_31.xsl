...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:for-each select="NACE-codes/NACE-code">
...
INSERT INTO NACE_codes (Code, DescriptionNL, DescriptionFR
        , Code_DmfAppl, Social_Maribel, BeginValidity, EndValidity)
VALUES("<xsl:value-of select="code"/>"
        , "<xsl:value-of select="description/nl"/>"
        , "<xsl:value-of select="description/fr"/>"
        , "<xsl:value-of select="code-DmfAppl"/>"
        , "<xsl:value-of select="social-maribel"/>"
