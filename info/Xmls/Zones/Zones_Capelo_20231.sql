...
SELECT "00988"
        ...
        , "0 : buitenlands diploma; 1 : Belgisch diploma"
        , ""
        , "numeric"
        , "1"
        , "required"
...
SELECT "00982"
        ...
        , "Voor een per uur of per prestatie betaald supplement komt het aantal opgegeven in deze zone overeen met het aantal uren of het aantal prestaties geleverd tijdens de in aanmerking genomen periode."
...
SELECT "00975"
        ...
        , "Dit is het aantal uren op basis van het welke de wedde van de werknemer berekend wordt of, als die werknemer een afwezigheid geniet, op basis van het welke zijn wedde zou berekend worden als hij die afwezigheid niet zou genieten."
        , "De uren worden uitgedrukt in honderdsten van uren.\nGeheel getal gelegen tussen [0;4800].\nVoorbeelden:\n- 38 uren 20 min. wordt uitgedrukt in de vorm: 3833\n- 38 uren wordt uitgedrukt in de vorm: 3800\n"
        , ""
        , "numeric"
        , "4"
        , "Verplicht indien de lijn 'Baremieke wedde' (FB 90420) samenvalt met ten minste één andere lijn 'Baremieke wedde' (FB 90420).\nVerplicht indien de zone 00976 'Aantal uren per week - volledige baremieke wedde' aanwezig is."
        , ""
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00975'
                          AND Version = '2023/1'
...
SELECT "00976"
        ...
        , "De uren worden uitgedrukt in honderdsten van uren. Geheel getal gelegen tussen [1;4800]."
        , ""
        , "numeric"
        , "4"
        , "Verplicht indien de zone 00975 'Aantal uren per week' aanwezig is. Verplicht indien de waarde verschilt van die in de zone 00049 'Gemiddeld aantal uren per week van de maatpersoon'."
        , ""
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00976'
                          AND Version = '2023/1'
...
SELECT "00967"
        ...
        , "0 = binnendienst; 1 = actieve dienst"
        , ""
        , "numeric"
        , "1"
        , "Facultatief"
...
SELECT "00968"
        ...
        , "1 = hoofdambt; 2 = bijbetrekking; 3 = bijbetrekking in het onderwijs met beperkt leerplan"
        , ""
        , "numeric"
        , "1"
        , "Verplicht indien de werknemer een functie uitoefent in het onderwijs, dit wil zeggen indien de zone 00961 'Type instelling van de overheidssector' ingevuld is en gelijk is aan één van de volgende waarden: 25, 26, 27, 52, 53, 54, 62, 63 of 64."
        , ""
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00968'
                          AND Version = '2023/1'
...
SELECT "00980"
        ...
        , "Positief geheel getal en element van [1; 99999999999]. Het basisbedrag wordt uitgedrukt in eurocenten."
        , ""
        , "numeric"
        , "11"
...
SELECT "00974"
        ...
        , "ScaleSalaryAmount"
        , "Baremieke wedde -- historische gegevens"
        , "Baremieke wedde (gekoppeld aan de spilindex 138,01) waarop de werknemer, rekening houdend met zijn geldelijke anciënniteit, recht heeft of recht zou hebben indien hij in dienst was gebleven."
        , "Positief geheel getal en element van [1; 99999999999]. Het bedrag wordt uitgedrukt in eurocenten."
        , ""
        , "numeric"
        , "11"
        , "required"
...
SELECT "00032"
        ...
        , "WorkerPostBox"
        , "Natuurlijke persoon"
        , "Nummer van een brievenbus. Busnummer in het adres van de werknemer.Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
        , ""
        , ""
        , "Alfanumeric"
        , "4"
...
SELECT "01002"
        ...
        , "Zie Bijlage 8 - Codificatie van arbeidstijdgegevens. Alleen de codes 31, 32, 33, 41, 42 en 43 zijn van toepassing om een niet-situeerbare afwezigheid aan te geven."
        , ""
        , "numeric"
        , "2"
        , "required "
...
SELECT "00970"
        ...
        , "Baremieke wedde - historische gegevens"
        , "Dit is de begindatum van de periode waarop de gegevens van de baremieke wedde betrekking hebben.\nIndien die gegevens onveranderd zijn, gaat het om de begindatum van het element van de loopbaangeschiedenis waarvan die gegevens afhangen."
        , ""
        , ""
        , "Alfanumeric"
        , "10"
...
SELECT "00972"
        ...
        , "Baremieke wedde - historische gegevens"
        , "Dit is de datum, uitgedrukt in jaar en maand, vanaf wanneer de door de werknemer opgebouwde geldelijke anciënniteit telt zonder onderbreking. De geldelijke anciëniteit laat toe om de trap van de weddeschaal te bepalen waarmee de basiswedde van de werknemer overeenstemt. De datum van ranginneming wijzigt enkel wanneer de geldelijke anciënniteit van de werknemer opnieuw moet worden berekend."
        , "Geldelijke anciënniteit. Jaar gelegen tussen [jaar van de aangifte - 100 ; jaar van de aangifte en laatste maand van het kalenderkwartaal]. Om aan te duiden dat de geldelijke anciënniteit niet van toepassing is, zal men '9999-12' opgeven. "
        , ""
        , "Alfanumeric"
        , "7"
        , "required"
        , "JJJJ-MM -- JJJJ is het jaar -- MM is de maand"
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00972'
                          AND Version = '2023/1'
);
...
SELECT "00637"
        ...
        , "Alfanumeric"
        , "60"
        , "Verplicht indien het attest gegevens voor de contactpersoon bevat en hierbij geen van de zones 00677 (TELEFOONNUMMER) en 00925 (AANVULLENDE ADRESGEGEVENS) is ingevuld (voorwaarden geldig wanneer de zone gebruikt wordt in het FB 90258 (Communicatie)); facultatief indien de zone behoort tot het FB 90437 (Informatie over het historisch attest)."
        , ""
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00637'
                          AND Version = '2023/1'
...
SELECT "00990"
        ...
        , "PSPSEmployerNbr"
        , "Identificatie van een werkgever in de loopbaangeschiedenis van de werknemer"
        , "Nummer dat een werkgever eenduidig identificeert binnen de FPD. Deze identificatie wordt gebruikt voor bepaalde werkgevers die niet meer bestaan en die in het kader van de samenstelling van het pensioendossier geïdentificeerd moeten worden aan de hand van een uniek nummer geleverd door de FPD."
...
SELECT "00168"
        ...
        , "WorkerBirthplace"
        , "Natuurlijke persoon"
        , "Naam van een gemeente. Naam van de gemeente waar de natuurlijke persoon geboren is.Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
        , ""
        , ""
        , "Alfanumeric"
        , "40"
...
SELECT "00034"
        ...
        , "WorkerCity"
        , "Natuurlijke persoon"
        , "Naam van een gemeente. Naam van de gemeente waar de werknemer gedomicilieerd is.Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
...
SELECT "00999"
        ...
        , "0 : de vestiging wordt niet gesubsidieerd; 1 : de vestiging wordt gesubsidieerd"
        , ""
        , "numeric"
        , "1"
        , "Facultatief"
...
SELECT "00296"
        ...
        , "Zie bijlage 11 - Identificatie van het formulier. Deze zone moet als waarde 'CDHG001' hebben."
        , ""
        , "Alfanumeric"
        , "7"
        , "required"
...
SELECT "00024"
        ...
        , "11"
        , "required"
        , "0 of NNNNNNNNNCC -- NNNNNNNNN is het nummer -- CC is het controlegetal."
WHERE NOT EXISTS (SELECT * FROM zones
                    WHERE Code = '00024'
                          AND Version = '2023/1'
);
...
SELECT "00027"
        ...
        , "WorkerInitial"
        , "Natuurlijke persoon"
        , "Initiaal van de tweede voornaam van een natuurlijk persoon. Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
...
SELECT "00059"
        ...
        , "1 = piloot; 2 = cabinepersoneel; 3 = andere"
        , ""
        , "numeric"
        , "1"
...
SELECT "00035"
        ...
        , "WorkerCountry"
        , "Natuurlijke persoon"
        , "Code die toelaat een land te identificeren. Code waarmee men het land kan identificeren waar de werknemer gedomicilieerd is.Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
        , "Zie Bijlage 5 - Lijst met landcodes."
        , ""
        , "numeric"
        , "5"
...
SELECT "00985"
        , "2023/1"
        , "28/02/2023"
        , "OFFICIËLE BENAMING VAN DE TITEL OF HET DIPLOMA"
        , "DegreeDenomination"
        , "Titels en diploma's"
        , "Officiële benaming die op de titel of het diploma staat"
        , ""
        , ""
        , "Alfanumeric"
        , "100"
...
SELECT "00981"
        ...
        , "Geheel getal en element van [1;99999]. Percentage uitgedrukt in honderdste percent (bv.: 21,52%  => 2152)."
        , ""
        , "numeric"
        , "5"
        , "Verplicht indien de referentie van de weddebijslag dit vereist."
...
SELECT "00962"
        ...
        , "Element van de loopbaangeschiedenis van de werknemer"
        , "Code die aanduidt tot welke personeelscategorie de werknemer behoort voor het in aanmerking nemen van de diensten in de berekening van het rustpensioen (tantième)."
...
SELECT "00033"
     ...
     Voor de in België gedomicilieerde werknemers moet men de officiële lijst met Belgische postcodenummers gebruiken.\nDeze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
        , ""
        , ""
        , "Alfanumeric"
        , "9"
...
SELECT "00998"
        ...
        , "Zie bijlage capelodhgs01 - Lijst van precaire contracten"
        , ""
        , "numeric"
        , "3"
...
SELECT "00615"
        ...
        , "Natuurlijke persoon"
        , "Referentie. Referentie van de natuurlijke persoon gebruikt door de werkgever of een erkend sociaal secretariaat.Deze zone wordt nooit gebruikt wanneer het gaat om een aangifte voor een attest Loopbaangeschiedenis."
...
SELECT "00989"
        ...
        , "1 : onderwijs met volledig leerplan - normaal uurrooster; 2 : onderwijs met volledig leerplan - verschoven uurrooster (inbegrepen centrale examenjury)"
        , ""
        , "numeric"
        , "1"
...
SELECT "01001"
        ...
        , "Weddebijslag - historische gegevens"
        , "Volgnummer van de weddebijslag. Dit is een volgnummer toegekend door de werkgever. Dit nummer moet uniek zijn binnen een lijn "Baremieke wedde - historische gegevens"."
        , "Geheel getal en element van [1;99]"
        , ""
        , "numeric"
        , "2"
...
SELECT "01006"
        ...
        , "EmployerSequenceNbr"
        , "Identificatie van een werkgever in de loopbaangeschiedenis van de werknemer"
        , "Volgnummer van een werkgever. Dit is een volgnummer toegekend door de aangever. Dit nummer moet uniek zijn binnen een aangifte."
        , "Geheel getal en element van [1;99]"
        , ""
        , "numeric"
        , "2"
